...
all match and the worktree is clean; otherwise return `BLOCKED`. Record the
base commit immediately. Run every implementation, validation, status, and
handoff mutation in the task worktree, never the primary checkout.
<!-- MERIDIAN:END -->
