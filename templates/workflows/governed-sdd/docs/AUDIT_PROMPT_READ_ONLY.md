...
<!-- MERIDIAN:BEGIN capability=audit-prompt v2 -->
...
10. The implementation-to-review-to-integration handoff matches `docs/PULL_REQUEST_POLICY.md`: deterministic task branches and linked worktrees, one sequential writer, one implementation push per review attempt, durable review records for `CHANGES_REQUESTED`, recorded-base ancestry verification, reviewer-integrator author override used only on the acceptance commit, serialized combined-tree validation and `main` merge commits, and cleanup only after successful integration without rebase/amend/cherry-pick/force recovery shortcuts.
