...
- Review: use `docs/CODE_REVIEW_PROMPT.md` in a fresh session that did not write the implementation; integrate only if all repository and forge gates are met. The reviewer never pushes the task branch again, never edits implementation artifacts while reviewing, and confirms the governed mode lock before making a status-only acceptance commit.
